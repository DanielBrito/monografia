...
def groupRegions(label, parent, gridLabel):
    ...
    selectedRegion = bpy.data.objects[gridLabel + ".001"]
    
    # Storing reference to region (to be used for extrusion from building):
    region = bpy.context.selected_objects[selectedRegion.pass_index]
    ...
    # Deselecting created region grid:
...
''' -------------- SIMULATING SELEX RULES (multiple_extrusions.slx): -------------- '''
...
selectToBeVolume(grid, grid.getRows(), grid.getColumns(), rowsIndex, columnsIndex)
